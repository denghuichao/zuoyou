 ...
 * SQLite数据库模板工具类
 * 
 * 该类提供了数据库操作常用的增删改变以及各种复杂条件匹配,分页,排序等操作
 * 
 * @see SQLiteDatabase
 */
public class SQLiteTemplate {
	...
	/**
	 * isTransaction 是否属于�?��事务 �?�?��isTransaction设为true
	 * �?��的SQLiteTemplate方法都不会自动关闭资�?�?��事务成功后手动关�?
	 * 
	 * @return
	 */
	...
	public <T> T queryForObject(RowMapper<T> rowMapper, String sql,
		...
		try {
			dataBase = dBManager.openDatabase();
			if(dataBase!=null)cursor = dataBase.rawQuery(sql, args);
	...
	public <T> List<T> queryForList(RowMapper<T> rowMapper, String sql,
		...
		try {
			dataBase = dBManager.openDatabase();
			if(dataBase!=null)cursor = dataBase.rawQuery(sql, selectionArgs);
	...
	public <T> Set<T> queryForSet(RowMapper<T> rowMapper, String sql,
			int startResult, int maxResult) {
		Cursor cursor = null;
		Set<T> list = null;
		try {
	 ...
	 * @param selection
	 *            查询条件子句，相当于select语句where关键字后面的部分，在条件子句允许使用占位�??"
	 * @param selectionArgs
	 *            对应于selection语句中占位符的�?，�?在数组中的位置与占位符在语句中的位置必须�?��，否则就会有异常
	 * @param groupBy
	...
	public <T> List<T> queryForList(RowMapper<T> rowMapper, String table,
		...
		try {
			dataBase = dBManager.openDatabase();
			if(dataBase!=null)cursor = dataBase.query(table, columns, selection, selectionArgs,
